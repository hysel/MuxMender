...
SDR test video frames matched within 2 ms, with original embedded subtitle packets
unchanged. External subtitles must be for the same cut. To auto-load after clean
naming, use a matching basename (for example `SDR test video.en.srt`); sidecar
...
process above 1 GiB measured resident/committed high-Episode B usage on Windows
(resident high-Episode B on Linux). If measurement is unavailable, it fails closed.
...
filenames, e.g. `Example Film (2020)/s7-example-film.1080.mkv` becomes
`Example Film (2020) - s7-example-film.1080.mkv`. Uncertain abbreviated release names
...
`Example.Film.2020.BluRay.720p.x264.DTS-WiKi.mkv` becomes
`Example Film (2020) - BluRay.720p.x264.DTS-WiKi.mkv`.
...
```powershell
python python/muxmender.py "C:\Media\s7-example-film.1080.mkv" --rename-title "Example Film (2020)" --rename-plan "C:\Plans\example-film.json"
```
