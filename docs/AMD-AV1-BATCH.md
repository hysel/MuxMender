...
QP80 was playback-tested on Battlestar and Andor; it is lossy, not universal quality.

## Plan first (default is dry-run)

```powershell
python -B python/amd_av1_batch.py "Y:\TV\Andor\Season 1" --output-dir "E:\MuxMender-TestOutputs\batches" --dry-run
```
...
```powershell
python -B python/amd_av1_batch.py "Y:\TV\Andor\Season 1" --output-dir "E:\MuxMender-TestOutputs\batches" --execute --max-files 3
```
