...
class Governor:
    ...
    def sample(self):
        ...
        try:
            ...
            if limit.exists() and limit.read_text().strip()!='max':
                remaining=(int(limit.read_text())-int(Path('/sys/fs/cgroup/memory.current').read_text()))/1024**3
    ...
    def admit(self, profile, active, now=None):
        ...
        if data.get('available_gib',0)<limits['memory_gib']+2:reason='Waiting for memory headroom'
