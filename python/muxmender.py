...
from media_naming import (clean_media_name, output_path, rename_identity,
                          check_rename_path, rename_release_suffix,
                          create_rename_plan, apply_rename_plan)
...
def progress_percent(line: str, duration_seconds: float) -> float | None:
    from encoder_progress import timestamp_percent
    return timestamp_percent(line, duration_seconds)
    ...
    try:
        ...
        while not eof or process.poll() is None:
            ...
            if activity.update(line):
                last_progress_at = time.monotonic()
                if last_advanced < 0 and activity.frames:
                    import job_tracking
                    job_tracking.stage_progress(None, None, detail=f'{activity.frames:,} frames processed; percentage unavailable')
...
def main(argv: list[str] | None = None) -> int:
    ...
    if supplied and supplied[0]=='automatic':
        from media_workflow import main as automatic_main
        return automatic_main(supplied[1:])
...
def cli():
    if sys.argv[1:2]==['automatic']:
        # The common engine owns tracking; avoid duplicate jobs, including help.
        return main()
