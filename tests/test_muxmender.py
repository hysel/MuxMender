...
class ExistingRenameTests(unittest.TestCase):
    def test_release_tail_preserved_and_second_preview_unchanged(self):
        for original, title, tail in [
            ('Example.2003.BluRay.720p.x264.DTS-WiKi.mkv', 'Example (2003)', 'BluRay.720p.x264.DTS-WiKi'),
            ('Example Adventure 2016 UHD BluRay HDR10 2160p Dts-HDMa7.1 HEVC-d3g.mkv', 'Example Adventure (2016)', 'UHD BluRay HDR10 2160p Dts-HDMa7.1 HEVC-d3g'),
    ...
    def test_folder_identity_preview_and_explicit_apply_preserve_contents(self):
        with tempfile.TemporaryDirectory() as tmp:
            folder = Path(tmp)/'Example Film (2011)'; folder.mkdir()
            video = folder/'s7-example.film.1080.mkv'; video.write_bytes(b'local dummy video')
            subtitle = folder/'s7-example.film.1080.en.srt'; subtitle.write_text('caption', encoding='utf-8')
            ...
            self.assertEqual(Path(plan['entries'][0]['destination']).name, 'Example Film (2011) - s7-example.film.1080.mkv')
            ...
            output = folder/'Example Film (2011) - s7-example.film.1080.mkv'
            self.assertEqual(output.read_bytes(), b'local dummy video')
            self.assertEqual(mm.rename_identity(output), original)
            self.assertEqual((folder/'Example Film (2011) - s7-example.film.1080.en.srt').read_text(encoding='utf-8'), 'caption')
...
class MuxMenderTests(unittest.TestCase):
    ...
    def test_preserve_release_movie_and_episode_output_names(self):
        ...
        for original, expected in [
            ('Example 1985 1080p AMZN WEB-DL DDP 5 1 H 264-PiRaTeS.mkv', 'Example 1985 1080p AMZN WEB-DL DDP 5 1 H 264-PiRaTeS.mkv'),
            ('Show.Name.S01E07.Episode.Title.2160p.WEB-DL.mp4', 'Show.Name.S01E07.Episode.Title.2160p.WEB-DL.mkv'),
            ('Example (2023).mkv', 'Example (2023).mkv'),
        ]:
