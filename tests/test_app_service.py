...
let staged=stagePresentation({settings:{mode:'replace'},workflow_stage:'encode',stage_percent:100});
assert(staged.title==='Step 3 of 6 · Encode','Stable numbered workflow');
assert(staged.check==='Current check complete · finishing this stage','100 percent is not job completion');
assert(stagePresentation({settings:{mode:'encode'},workflow_stage:'validate'}).title==='Step 4 of 4 · Validate','Safe copy excludes replacement');
assert(stagePresentation({settings:{mode:'test'},workflow_stage:'compare'}).steps.length===2,'Sample-only workflow');
assert(stagePresentation({settings:{mode:'replace'},stage_percent:null}).check==='Current check: measuring','No invented percentage');
assert(stagePresentation({settings:{mode:'replace'},stage_percent:102}).check==='Current check: measuring','Reject invalid percentage');
assert(stagePresentation({settings:{mode:'replace'},workflow_stage:'cleanup'}).title==='Step 6 of 6 · Clean up','Cleanup visible');
